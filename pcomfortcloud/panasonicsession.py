...
class PanasonicSession:
    ...
    def __init__(
        ...
        settingsFileName="~/.panasonic-settings",
        raw=False,
    ):
        ...
        self._authentication = PanasonicAuthentication(
            client, self._settings, self._app_version
        )
        self._raw = raw
    ...
    async def start_session(self):
        ...
        if not self._settings.is_access_token_valid:
            ...
            try:
                ...
                _LOGGER.debug(
                    "Failed to refresh token, trying to reauthenticate", exc_info=ex
                )
                await self._authentication.authenticate(self._username, self._password)
        if not self._settings.is_access_token_valid:
    ...
    async def execute_post(
        self, url, json_data, function_description, expected_status_code
    ):
        async with self._request_semaphore:
            ...
            await check_response(
                response, function_description, expected_status_code, payload=json_data
            )
            response_text = await response.text()
            _LOGGER.debug(
                "POST url: %s, data: %s, response: %s", url, json_data, response_text
            )
